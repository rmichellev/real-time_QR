...
type ProjectType = "landing" | "web" | "ecommerce" | "app" | "sistema";
...
function inferProjectType(description: string): ProjectType {
  ...
  if (text.includes("ecommerce") || text.includes("tienda") || text.includes("carrito") || text.includes("pago")) return "ecommerce";
  if (text.includes("app") || text.includes("móvil") || text.includes("movil") || text.includes("android") || text.includes("ios")) return "app";
  if (text.includes("sistema") || text.includes("dashboard") || text.includes("inventario") || text.includes("crm") || text.includes("reservas")) return "sistema";
  if (text.includes("landing") || text.includes("publicidad") || text.includes("promocional")) return "landing";
...
function getComplexityMultiplier(complexity: Complexity): number {
  ...
  if (complexity === "alta") return 1.45;
  return 1;
}

function buildScreens(projectType: ProjectType, includeQr: boolean): Screen[] {
  const base: Screen[] = [
  ...
  const extras: Record<ProjectType, Screen[]> = {
    landing: [
...
function buildStages(screenCount: number, complexity: Complexity, includeQr: boolean): Stage[] {
